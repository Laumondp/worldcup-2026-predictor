...
function matchName(apiName, standingName) {
  return norm(apiName) === norm(standingName);
}

export default async function handler(req, res) {
  // Build standings map from authoritative group data
  const standings = {};
  ...
  // Try to fetch live results from FIFA API
  try {
    ...
    for (const m of results) {
      ...
      for (const groupTeams of Object.values(standings)) {
        const homeEntry = groupTeams.find(t => matchName(homeName, t.name));
        const awayEntry = groupTeams.find(t => matchName(awayName, t.name));
